...
def getTradeAnalysisDf(analyzer, outputXLSXToFolder: str = None):
    total_open = analyzer.total.open
    total_closed = analyzer.total.closed
    total_won = analyzer.won.total
    total_lost = analyzer.lost.total
    ...
    if outputXLSXToFolder != None:
        fileName = "TradeAnalysis.xlsx"
        assert outputXLSXToFolder != None
        Helper().outputXLSX(resultDF, folderName=outputXLSXToFolder, fileName=fileName)
    return resultDF
